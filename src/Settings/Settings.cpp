...
//
// videogamescoin is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// videogamescoin is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with videogamescoin.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "videogamescoin.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("videogamescoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("videogamescoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("videogamescoinwallet.desktop");
  ...
  res = autorunSettings.contains("videogamescoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("videogamescoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.videogamescoin.videogamescoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=videogamescoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("videogamescoinWallet", appPath);
    } else {
      autorunSettings.remove("videogamescoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\videogamescoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:videogamescoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\videogamescoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\videogamescoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("videogamescoinwallet.cfg"));
